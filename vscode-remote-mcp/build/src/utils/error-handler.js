...
exports.ErrorRecoveryManager = exports.RateLimiter = exports.ConnectionManager = exports.DefaultRecoveryActions = exports.ErrorCode = exports.ErrorCategory = void 0;
...
exports.ErrorCode = {
    // Protocol errors
    INVALID_MESSAGE_FORMAT: { category: exports.ErrorCategory.PROTOCOL, retryable: true },
    UNKNOWN_MESSAGE_TYPE: { category: exports.ErrorCategory.PROTOCOL, retryable: true },
    MISSING_REQUIRED_FIELD: { category: exports.ErrorCategory.PROTOCOL, retryable: true },
    INVALID_FIELD_VALUE: { category: exports.ErrorCategory.PROTOCOL, retryable: true },
    // Authentication errors
    AUTH_FAILED: { category: exports.ErrorCategory.AUTHENTICATION, retryable: false },
    AUTH_EXPIRED: { category: exports.ErrorCategory.AUTHENTICATION, retryable: false },
    AUTH_REQUIRED: { category: exports.ErrorCategory.AUTHENTICATION, retryable: false },
    AUTH_INSUFFICIENT_PERMISSIONS: { category: exports.ErrorCategory.AUTHENTICATION, retryable: false },
    // Session errors
    SESSION_NOT_FOUND: { category: exports.ErrorCategory.SESSION, retryable: true },
    SESSION_ALREADY_EXISTS: { category: exports.ErrorCategory.SESSION, retryable: false },
    SESSION_JOIN_REJECTED: { category: exports.ErrorCategory.SESSION, retryable: false },
    SESSION_FULL: { category: exports.ErrorCategory.SESSION, retryable: false },
    // Resource errors
    RESOURCE_NOT_FOUND: { category: exports.ErrorCategory.RESOURCE, retryable: true },
    RESOURCE_LOCKED: { category: exports.ErrorCategory.RESOURCE, retryable: true },
    RESOURCE_LIMIT_EXCEEDED: { category: exports.ErrorCategory.RESOURCE, retryable: false },
    RESOURCE_CONFLICT: { category: exports.ErrorCategory.RESOURCE, retryable: false },
    // Server errors
    SERVER_ERROR: { category: exports.ErrorCategory.SERVER, retryable: true },
    SERVER_OVERLOADED: { category: exports.ErrorCategory.SERVER, retryable: true },
    SERVER_MAINTENANCE: { category: exports.ErrorCategory.SERVER, retryable: false },
    SERVER_SHUTTING_DOWN: { category: exports.ErrorCategory.SERVER, retryable: false },
    // Client errors
    CLIENT_TIMEOUT: { category: exports.ErrorCategory.CLIENT, retryable: true },
    CLIENT_DISCONNECTED: { category: exports.ErrorCategory.CLIENT, retryable: true },
    CLIENT_RATE_LIMITED: { category: exports.ErrorCategory.CLIENT, retryable: true },
    CLIENT_VERSION_UNSUPPORTED: { category: exports.ErrorCategory.CLIENT, retryable: false },
    // Test error
    TEST_ERROR: { category: exports.ErrorCategory.CLIENT, retryable: true }
};
/**
 * Default recovery actions for specific error codes
 * @enum {Object}
 */
exports.DefaultRecoveryActions = {
...
function createErrorMessage(code, message, id, relatedTo, fatal = false, details, recoveryAction) {
    ...
    if (exports.ErrorCode[code] && exports.ErrorCode[code].category) {
        errorMessage.payload.category = exports.ErrorCode[code].category;
    }
    return errorMessage;
}
/**
...
function validateErrorMessage(message) {
    ...
    if (message.payload.category && !Object.values(exports.ErrorCategory).includes(message.payload.category)) {
...
function isRetryableError(error) {
    ...
    if (error && error.payload && error.payload.code && exports.ErrorCode[error.payload.code]) {
        return exports.ErrorCode[error.payload.code].retryable;
    }
...
function getRecoveryAction(error, includeDefaults = false) {
    ...
    if (includeDefaults && error && error.payload && error.payload.code && exports.DefaultRecoveryActions[error.payload.code]) {
        return exports.DefaultRecoveryActions[error.payload.code];
    }
    return null;
}
/**
 ...
 */
function handleError(error, clientId, connectionManager, clientStateModel) {
    // Get log level from environment config
    const logLevel = env_1.default.log.level;
    ...
    if (error.code === 'CORS_ERROR') {
        const corsEnabled = env_1.default.security.corsEnabled;
        if (corsEnabled) {
            const allowedOrigins = env_1.default.security.allowedOrigins;
 ...
 * @param {Object} clientStateModel - The client state model
 */
async function handleAuthError(error, clientId, connectionManager, clientStateModel) {
    // Check if authentication is enabled from config
    const authEnabled = env_1.default.auth.enabled;
    // Get client from connection manager
    const client = connectionManager.getClient(clientId);
    if (!client) {
    ...
    if (error.code === 'INVALID_TOKEN') {
        authErrorResponse.payload.expectedToken = env_1.default.auth.token;
    }
    // Send error to client
...
async function handleRequestError(error, clientId, requestId, connectionManager, clientStateModel) {
    ...
    if (error.code === 'COMMAND_TIMEOUT') {
        requestErrorResponse.payload.timeoutMs = env_1.default.command.timeoutMs;
    }
    // Send error to client
...
async function handleServerError(error, connectionManager) {
    ...
    const serverName = env_1.default.server.name;
    const serverVersion = env_1.default.server.version;
