...
const health_monitor_1 = require("../../src/swarm/health-monitor");
const migration_manager_1 = require("../../src/swarm/migration-manager");
...
const mockProvider = {
    ...
    stopInstance: jest.fn().mockResolvedValue({
        ...
        name: 'test-instance',
        providerType: provider_types_1.ProviderType.DOCKER,
        status: provider_types_1.InstanceStatus.STOPPED,
        config: {},
        network: {},
        resources: {},
        metadata: {},
...
describe('SwarmController', () => {
    ...
    describe('initialize', () => {
        it('should initialize the swarm controller', async () => {
            ...
            expect(provider_factory_1.ProviderFactory.createProvider).toHaveBeenCalledTimes(2);
        ...
        describe('startInstance', () => {
            ...
            beforeEach(async () => {
                ...
                mockGetInstance.mockImplementation((instanceId) => {
                    if (instanceId === 'non-existent-instance') {
                        return null;
                    }
                    return {
            ...
            it('should start an instance successfully', async () => {
                ...
                const mockUpdateInstance = instance_registry_1.InstanceRegistry.mock.results[0].value.updateInstance;
                const instance = await swarmController.startInstance('test-instance');
                expect(instance).toBeDefined();
                expect(instance.id).toBe('test-instance');
                expect(instance.status).toBe(provider_types_1.InstanceStatus.RUNNING);
            ...
            it('should throw an error if provider is not found', async () => {
                ...
                const localSwarmController = new swarm_controller_1.SwarmController({
                    ...testConfig,
                    providers: [
                        {
                            type: provider_types_1.ProviderType.DOCKER,
                ...
                mockGetInstance.mockImplementation((instanceId) => {
                    return {
                        id: instanceId,
                        name: 'test-instance',
                        providerType: provider_types_1.ProviderType.FLYIO, // Different provider than what's available
                        status: provider_types_1.InstanceStatus.STOPPED,
                        config: {},
                        network: {},
                        resources: {},
            ...
            it('should handle provider failures when starting an instance', async () => {
                ...
                mockProvider.startInstance.mockRejectedValueOnce(new Error('Provider failure'));
    ...
    describe('createInstance', () => {
        ...
        it('should create a new instance', async () => {
            // Get the mock implementation
            const mockRegisterInstance = instance_registry_1.InstanceRegistry.mock.results[0].value.registerInstance;
            const config = {
                name: 'test-instance',
                image: 'vscode:latest',
            ...
            expect(instance.providerType).toBe(provider_types_1.ProviderType.DOCKER);
            expect(instance.status).toBe(provider_types_1.InstanceStatus.RUNNING);
    ...
    describe('stopInstance', () => {
        ...
        describe('SwarmController - Health Checks', () => {
            ...
            describe('checkInstanceHealth', () => {
                beforeEach(async () => {
                    ...
                    mockGetInstance = instance_registry_1.InstanceRegistry.mock.results[0].value.getInstance;
                    mockCheckInstanceHealth = health_monitor_1.HealthMonitor.mock.results[0].value.checkInstanceHealth;
                    // Mock registry getInstance
                    mockGetInstance.mockImplementation((instanceId) => {
                        if (instanceId === 'non-existent-instance') {
                            return null;
                ...
                it('should check instance health successfully', async () => {
                    ...
                    expect(health.status).toBe(config_1.HealthStatus.UNKNOWN); // Initial status before health check completes
            ...
            describe('createMigrationPlan', () => {
                ...
                beforeEach(async () => {
                    ...
                    mockFlyProvider = {
                        ...
                        createInstance: jest.fn().mockImplementation((config) => {
                            const instance = {
                                ...
                                name: config.name,
                                providerType: provider_types_1.ProviderType.FLYIO,
                    ...
                    // Get the mock implementations
                    mockGetInstance = instance_registry_1.InstanceRegistry.mock.results[0].value.getInstance;
                    mockCreateMigrationPlan = migration_manager_1.MigrationManager.mock.results[0].value.createMigrationPlan;
                    // Mock registry getInstance
                    mockGetInstance.mockImplementation((instanceId) => {
                        if (instanceId === 'non-existent-instance') {
                            return null;
                ...
                it('should create a migration plan successfully', async () => {
                    ...
                    const expectedPlan = {
                        ...
                        targetProviderType: provider_types_1.ProviderType.FLYIO,
                    ...
                    const plan = await swarmController.createMigrationPlan('test-instance', provider_types_1.ProviderType.FLYIO);
                    expect(plan).toBeDefined();
                    expect(plan.id).toBe('test-migration');
                    expect(plan.sourceInstanceId).toBe('test-instance');
                    expect(plan.sourceProviderType).toBe(provider_types_1.ProviderType.DOCKER);
                    expect(plan.targetProviderType).toBe(provider_types_1.ProviderType.FLYIO);
                    expect(mockGetInstance).toHaveBeenCalledWith('test-instance');
                    expect(mockCreateMigrationPlan).toHaveBeenCalledWith('test-instance', provider_types_1.ProviderType.FLYIO, undefined);
                });
                it('should create a migration plan with custom options', async () => {
                    ...
                    const expectedPlan = {
                        ...
                        sourceProviderType: provider_types_1.ProviderType.DOCKER,
                        targetProviderType: provider_types_1.ProviderType.FLYIO,
                        strategy: config_1.MigrationStrategy.CREATE_THEN_STOP,
                        keepSource: true,
                        startTarget: false,
                        timeoutSeconds: 600,
                        createdAt: new Date(),
                    ...
                    const plan = await swarmController.createMigrationPlan('test-instance', provider_types_1.ProviderType.FLYIO, options);
                    expect(plan).toBeDefined();
                    expect(plan.strategy).toBe(config_1.MigrationStrategy.CREATE_THEN_STOP);
                    expect(plan.keepSource).toBe(true);
                    expect(plan.startTarget).toBe(false);
                    expect(plan.timeoutSeconds).toBe(600);
                    expect(mockCreateMigrationPlan).toHaveBeenCalledWith('test-instance', provider_types_1.ProviderType.FLYIO, options);
                });
                describe('startMigration', () => {
                    let mockStartMigration;
                    let mockGetMigrationPlan;
                    beforeEach(async () => {
                        await swarmController.initialize();
                        // Get the mock implementations
                        mockStartMigration = migration_manager_1.MigrationManager.mock.results[0].value.startMigration;
                        mockGetMigrationPlan = migration_manager_1.MigrationManager.mock.results[0].value.getMigrationPlan;
                        // Mock getMigrationPlan to return a valid plan
                        mockGetMigrationPlan.mockImplementation((migrationId) => {
                    ...
                    describe('startMigration', () => {
                        ...
                        it('should handle non-existent migration plans', async () => {
                            ...
                            mockStartMigration.mockImplementation((migrationId) => {
                                if (migrationId === 'non-existent-migration') {
                    ...
                    describe('cancelMigration', () => {
                        let mockCancelMigration;
                        let mockGetMigrationPlan;
                        beforeEach(async () => {
                            await swarmController.initialize();
                            // Get the mock implementations
                            mockCancelMigration = migration_manager_1.MigrationManager.mock.results[0].value.cancelMigration;
                            mockGetMigrationPlan = migration_manager_1.MigrationManager.mock.results[0].value.getMigrationPlan;
                            // Mock getMigrationPlan to return a valid plan
                            mockGetMigrationPlan.mockImplementation((migrationId) => {
                                if (migrationId === 'non-existent-migration') {
                        ...
                        it('should handle non-existent migration plans', async () => {
                            // Mock migration manager to throw an error for non-existent plans
                            mockCancelMigration.mockImplementation((migrationId) => {
                                if (migrationId === 'non-existent-migration') {
                                    throw new Error(`Migration plan ${migrationId} not found`);
                                }
                    ...
                    describe('getSwarmStatus', () => {
                        ...
                        beforeEach(async () => {
                            ...
                            mockGetInstanceCount = instance_registry_1.InstanceRegistry.mock.results[0].value.getInstanceCount;
                            // Mock instance counts
                            mockGetInstanceCount.mockImplementation((providerType) => {
                                if (providerType === provider_types_1.ProviderType.DOCKER) {
                                    return 3;
                                }
                                else if (providerType === provider_types_1.ProviderType.FLYIO) {
                                    return 2;
                                }
                        ...
                        it('should return swarm status with instance counts and provider statistics', () => {
                            ...
                            expect(status.providers).toHaveLength(Object.values(provider_types_1.ProviderType).length);
                            // Check Docker provider status
                            const dockerProvider = status.providers.find(p => p.type === provider_types_1.ProviderType.DOCKER);
                            ...
                            const flyProvider = status.providers.find(p => p.type === provider_types_1.ProviderType.FLYIO);
                            ...
                            expect(mockGetInstanceCount).toHaveBeenCalledWith(provider_types_1.ProviderType.DOCKER);
                            expect(mockGetInstanceCount).toHaveBeenCalledWith(provider_types_1.ProviderType.FLYIO);
                        });
                        ...
                        it('should throw an error if controller is not initialized', () => {
                            ...
                            const uninitializedController = new swarm_controller_1.SwarmController(testConfig);
                ...
                it('should throw an error if source instance does not exist', async () => {
                    await expect(swarmController.createMigrationPlan('non-existent-instance', provider_types_1.ProviderType.FLYIO)).rejects.toThrow('Source instance non-existent-instance not found');
                    expect(mockGetInstance).toHaveBeenCalledWith('non-existent-instance');
                ...
                it('should throw an error if target provider is not found', async () => {
                    ...
                    await localSwarmController.initialize();
                    await expect(localSwarmController.createMigrationPlan('test-instance', provider_types_1.ProviderType.FLYIO)).rejects.toThrow('Target provider flyio not found or not initialized');
                    expect(mockGetInstance).toHaveBeenCalledWith('test-instance');
                    expect(mockCreateMigrationPlan).not.toHaveBeenCalled();
                });
                it('should handle migration manager failures', async () => {
                    ...
                    await expect(swarmController.createMigrationPlan('test-instance', provider_types_1.ProviderType.FLYIO)).rejects.toThrow('Migration manager failure');
                    expect(mockGetInstance).toHaveBeenCalledWith('test-instance');
                    expect(mockCreateMigrationPlan).toHaveBeenCalledWith('test-instance', provider_types_1.ProviderType.FLYIO, undefined);
                });
            });
        ...
        describe('SwarmController - Recovery', () => {
            ...
            describe('recoverInstance', () => {
                beforeEach(async () => {
                    ...
                    // Get the mock implementations
                    mockGetInstance = instance_registry_1.InstanceRegistry.mock.results[0].value.getInstance;
                    mockRecoverInstance = health_monitor_1.HealthMonitor.mock.results[0].value.recoverInstance;
                    // Mock registry getInstance
                    mockGetInstance.mockImplementation((instanceId) => {
                        if (instanceId === 'non-existent-instance') {
                            return null;
                ...
                it('should handle recovery failures', async () => {
                    ...
                    mockProvider.stopInstance.mockRejectedValueOnce(new Error('Provider failure'));
                    const success = await swarmController.recoverInstance('test-instance');
                    expect(success).toBe(false);
                    expect(mockGetInstance).toHaveBeenCalledWith('test-instance');
                    expect(mockProvider.stopInstance).toHaveBeenCalledWith('test-instance');
        ...
        it('should stop an instance successfully', async () => {
            ...
            expect(instance).toBeDefined();
            expect(instance.id).toBe('test-instance');
            expect(instance.status).toBe(provider_types_1.InstanceStatus.STOPPED);
            expect(mockGetInstance).toHaveBeenCalledWith('test-instance');
    ...
    describe('listInstances', () => {
        ...
        const testInstances = [
            ...
            },
            {
                ...
                providerType: provider_types_1.ProviderType.FLYIO,
                status: provider_types_1.InstanceStatus.FAILED,
                config: {},
                network: {},
                resources: {},
        ...
        let mockListInstances;
        beforeEach(async () => {
            await swarmController.initialize();
            // Get the mock implementation
            mockListInstances = instance_registry_1.InstanceRegistry.mock.results[0].value.listInstances;
        ...
        it('should filter instances by status (single value)', async () => {
            const instances = await swarmController.listInstances({ status: provider_types_1.InstanceStatus.RUNNING });
            expect(instances).toHaveLength(2);
            expect(instances[0].id).toBe('instance-1');
            expect(instances[1].id).toBe('instance-2');
            expect(instances.every(i => i.status === provider_types_1.InstanceStatus.RUNNING)).toBe(true);
        });
        it('should filter instances by status (multiple values)', async () => {
            const instances = await swarmController.listInstances({
                status: [provider_types_1.InstanceStatus.RUNNING, provider_types_1.InstanceStatus.FAILED]
            });
            ...
            expect(instances.every(i => i.status === provider_types_1.InstanceStatus.RUNNING || i.status === provider_types_1.InstanceStatus.FAILED)).toBe(true);
        });
    ...
    describe('deleteInstance', () => {
        let mockGetInstance;
        let mockRemoveInstance;
        beforeEach(async () => {
            await swarmController.initialize();
            // Get the mock implementations
            mockGetInstance = instance_registry_1.InstanceRegistry.mock.results[0].value.getInstance;
            mockRemoveInstance = instance_registry_1.InstanceRegistry.mock.results[0].value.removeInstance;
            // Mock registry getInstance
            mockGetInstance.mockImplementation((instanceId) => {
                if (instanceId === 'non-existent-instance') {
                    return null;
        ...
        it('should not remove instance from registry if provider deletion fails', async () => {
            ...
            mockProvider.deleteInstance.mockResolvedValueOnce(false);
            const success = await swarmController.deleteInstance('test-instance');
            expect(success).toBe(false);
            expect(mockGetInstance).toHaveBeenCalledWith('test-instance');
            expect(mockProvider.deleteInstance).toHaveBeenCalledWith('test-instance');
        ...
        it('should handle provider failures when deleting an instance', async () => {
            ...
            mockProvider.deleteInstance.mockRejectedValueOnce(new Error('Provider failure'));
    ...
    describe('updateInstance', () => {
        ...
        it('should update an instance successfully', async () => {
            ...
            mockProvider.updateInstance.mockImplementation((instanceId, config) => {
                return Promise.resolve({
                    id: instanceId,
        ...
        it('should handle provider failures when updating an instance', async () => {
            ...
            mockProvider.updateInstance.mockRejectedValueOnce(new Error('Provider failure'));
            const updateConfig = {
                name: 'updated-instance'
            };
...
describe('listMigrationPlans', () => {
    ...
    beforeEach(async () => {
        ...
        await swarmController.initialize();
        // Get the mock implementation
        mockListMigrationPlans = migration_manager_1.MigrationManager.mock.results[0].value.listMigrationPlans;
    });
    ...
    it('should filter migration plans by status', () => {
        ...
        const plans = swarmController.listMigrationPlans('completed');
...
describe('getProviderCapabilities', () => {
    ...
    it('should get capabilities for a valid provider type', () => {
        const dockerCapabilities = swarmController.getProviderCapabilities(provider_types_1.ProviderType.DOCKER);
    ...
    it('should get capabilities for a different provider type', () => {
        const flyCapabilities = swarmController.getProviderCapabilities(provider_types_1.ProviderType.FLYIO);
    ...
    it('should return null for an invalid provider type', async () => {
        ...
        const capabilities = localSwarmController.getProviderCapabilities(provider_types_1.ProviderType.FLYIO);
    ...
    it('should throw an error if controller is not initialized', () => {
        ...
        expect(() => {
            uninitializedController.getProviderCapabilities(provider_types_1.ProviderType.DOCKER);
        }).toThrow('Swarm controller not initialized');
    });
    it('should retrieve capabilities from multiple providers', async () => {
        // Create a swarm controller with multiple providers
        const multiProviderController = new swarm_controller_1.SwarmController({
            ...testConfig,
            providers: [
                {
                    type: provider_types_1.ProviderType.DOCKER,
        ...
        const dockerCapabilities = multiProviderController.getProviderCapabilities(provider_types_1.ProviderType.DOCKER);
        // Get capabilities for FlyIO provider
        const flyioCapabilities = multiProviderController.getProviderCapabilities(provider_types_1.ProviderType.FLYIO);
    ...
    it('should handle disabled providers when retrieving capabilities', async () => {
        ...
        const controllerWithDisabledProvider = new swarm_controller_1.SwarmController({
            ...testConfig,
            providers: [
                {
                    type: provider_types_1.ProviderType.DOCKER,
        ...
        const dockerCapabilities = controllerWithDisabledProvider.getProviderCapabilities(provider_types_1.ProviderType.DOCKER);
        // Get capabilities for FlyIO provider (disabled)
        const flyioCapabilities = controllerWithDisabledProvider.getProviderCapabilities(provider_types_1.ProviderType.FLYIO);
...
describe('dispose', () => {
    it('should dispose all resources properly', async () => {
        ...
        const swarmController = new swarm_controller_1.SwarmController(testConfig);
        // Initialize the controller
        await swarmController.initialize();
        // Call dispose method
    ...
    it('should handle errors during dispose gracefully', async () => {
        ...
        const healthMonitor = swarmController.healthMonitor;
        ...
        expect(mockConsoleError).toHaveBeenCalledWith('Error disposing health monitor:', expect.objectContaining({
...
//# sourceMappingURL=swarm-controller.test.js.map